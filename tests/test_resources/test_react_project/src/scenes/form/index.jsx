...
const userSchema = yup.object().shape({
    ...
    email: yup.string().email("Invalid email").required("required"),  // email validation is build-in yup
    contact: yup.string().matches(phoneRegExp, "PhoneNumber is not Valid").required("required"), //Regex validation and required
                ...
                {({values, errors, touched, handleBlur, handleChange, handleSubmit}) => (
                    <form onSubmit={handleSubmit}>
                        <Box display='grid' gap='30px' gridTemplateColumns='repeat(4, minmax(0, 1fr)' sx={{"& > div" : {gridColumn: isNonMobile ? undefined : "span 4"}}}>
                            ...
                            <TextField fullWidth variant='filled' type='text' label='Contact' onBlur={handleBlur} onChange={handleChange} value={values.contact} name='contact' error={!!touched.contact && !!errors.contact} helperText={touched.contact && errors.contact} sx={{gridColumn: 'span 4'}}/>
